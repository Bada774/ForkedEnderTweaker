	...
	private static String generateId(IIngredient input, ItemStack output, ResourceLocation... entities) {
		...
		if (input != null) {
			...
			if (items != null && !items.isEmpty()) {
				inputName = items.get(0).getDisplayName();
			}
		}

		String outputName = "unknown";
		if (output != null && output.getItem().getRegistryName() != null) {
			outputName = output.getDisplayName() + "_" + output.getItemDamage();
		}
		...
		return String.format("FET_SoulBinder: %s to %s with %s", inputName, outputName, entityName);
	}
